...
**Location exclusion matches places, not words.** Every distinct location
...
excluded locations are rows of the same table: a country criterion takes every
city in it, a city criterion that city, a bare Remote criterion remote
postings. The word match on the raw string stays beside it, so a string not
yet classified still excludes by word and never excludes by place. A wrong
row is a PUT to /admin/locations, never a deploy; the sweep never re-asks
about a string that has a row.
