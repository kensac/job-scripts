...
/** @param {import("./types").AdapterContext} context */
export function createAdapter(context) {
  const FORM = "form#application-form";
  const sleep = (ms) => context.operation.sleep(ms);
  ...
  async function questions() {
    ...
    const res = await context.getPublicJson(`https://${apiHost()}/v1/boards/${p.board}/jobs/${p.id}?questions=true`);
