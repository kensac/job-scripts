import { isStopped } from "../runtime/operation";
import { commitControl } from "./dom";
// The config-driven reader. adapters/recipes/<ATS>.json (generated by
...
export function createRecipeAdapter(configs, context) {
  ...
  async function typeInto(el, value, opts, blur) {
    if (!takesValue(el)) return false;
    const t = events(opts);
    ...
    if (blur) await commitControl(el, context.operation, t);
    return true;
  }
  ...
  async function runMethod(method, el, value, file, opts) {
    ...
    switch (m) {
      ...
      case "ui5": {
        ...
        await typeInto(el, value, opts, false);
        el.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", code: "Enter", keyCode: 13, bubbles: true }));
        el.dispatchEvent(new KeyboardEvent("keyup", { key: "Enter", code: "Enter", keyCode: 13, bubbles: true }));
        await commitControl(el, context.operation, events(opts));
        return true;
      }
      case "tinyMCE":
  ...
  async function fillInner(field, value, file, root) {
    if (field._spec) {
      ...
      if (field._person) ok = false;
        ...
        const items = ((context.profile)[field.fact] || []).filter((x) => x && (x.company || x.school));
  ...
  const buttonAt = (paths) => {
    const node = first(paths || [])?.el;
    return node instanceof HTMLElement ? node : null;
  };
  const nextButton = () => buttonAt(cfg.continue);
  const submitButton = () => buttonAt(cfg.submit);
