...
def _chunk(uid=7, prompt_hash="criteria", status="running"):
    return make_task(
        ...
        {"user_id": uid, "filter": {"prompt_hash": prompt_hash},
...
def test_decision_that_arrived_after_planning_prevents_submission():
    task = _chunk()
    add_ai_result("https://posting", "passed", None, "custom",
                  prompt_hash="criteria", model="gpt-6-luna")
...
def test_completed_owner_is_replaced_by_its_decision():
    first = _chunk()
    second = _chunk()
    add_ai_result("https://posting", "rejected", None, "custom",
                  prompt_hash="criteria", model="gpt-6-luna")
...
def test_other_users_revisions_and_models_do_not_supply_a_decision():
    ...
    add_ai_result("https://posting", "passed", None, "custom",
                  prompt_hash="criteria", model="gpt-5.6-luna")
    assert _blocked(_chunk()) == set()
