...
def test_public_discovery_filters_apply_before_pagination(client, query, expected):
    ...
    db.execute("UPDATE jobs SET locations=ARRAY['Toronto Remote'], comp_currency='CAD' WHERE id=%s", (ids[1],))
    db.execute("UPDATE jobs SET comp_max=NULL WHERE id=%s", (ids[2],))
    db.execute("INSERT INTO locations (text, remote) VALUES ('Toronto Remote', true), ('New York', false)")
