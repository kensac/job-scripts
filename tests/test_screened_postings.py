"""What a title pattern screens out is kept, and a candidate pattern can be
judged against it before it goes live."""
...
def test_what_the_pattern_drops_is_kept_and_ages_out_when_the_board_stops_listing_it(
    ...
    screened = {
        r["title"]: r for r in db.query("SELECT * FROM screened_postings WHERE source = 'acme'")
    }
    assert set(screened) == {
        "Software Engineer",
    ...
    db.execute(
        "UPDATE screened_postings SET last_seen_at = now() - interval '40 days' "
    ...
    assert {r["title"] for r in db.query("SELECT title FROM screened_postings")} == {
        "Software Engineer",
        "Senior Software Engineer",
    }
