...
def _fill(client, headers):
    response = client.post("/v1/user/apply/resolve", headers=headers, json={
...
def test_fill_review_is_owned_and_submitted_fills_are_immutable(client, user_headers, other_user_headers):
    fill_id = _fill(client, user_headers)
    path = f"/v1/user/apply/fills/{fill_id}"
    ...
    assert client.post(path + "/submitted", headers=user_headers, json={"fields": []}).status_code == 200
    assert client.put(path + "/answer", headers=user_headers, json=edit).status_code == 409
...
def test_newer_edit_and_submission_fence_late_results(client, user_headers):
    ...
    assert not fill_answers.complete(user_id, fill_id, token, ["why"], {"why": "Old"},
                                     {"why": "Old"}, [], review_only=True)
    token, _ = fill_answers.reserve(user_id, fill_id, ["why"], None)
    client.post(f"/v1/user/apply/fills/{fill_id}/submitted", headers=user_headers, json={"fields": []})
    assert not fill_answers.complete(user_id, fill_id, token, ["why"], {"why": "Old"},
                                     {"why": "Old"}, [], review_only=True)
