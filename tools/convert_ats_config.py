...
Writes one file per ATS under extension/adapters/recipes/ (the url match patterns, the
fields with their selectors and fill methods, the continue, submit and
success paths). The extension build derives one content script per table,
so a page loads only its own table. Names after
...
def main(argv: list[str]) -> int:
    ...
    out_dir = ROOT / "extension" / "adapters" / "recipes"
    ...
    for c in configs:
        ...
        (out_dir / f"{c['name']}.json").write_text(body + "\n")
